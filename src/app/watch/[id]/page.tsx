import { getMovieDetails, getMovieCredits, getMovieImages, getMovieCollection } from "@/lib/tmdb";
...
export default async function WatchPage({
  ...
  let collectionMovies: import("@/types/tmdb").Movie[] = [];
  if (movie.collectionId) {
    collectionMovies = await getMovieCollection(movie.collectionId);
  }
  ...
  return (
    <main className="w-full min-h-screen bg-gradient-to-br from-[#130b18] via-[#0a050a] to-[#11050a] overflow-x-hidden font-sans">
      ...
      {collectionMovies.length > 1 && (
        <CollectionTimeline 
          collectionName={movie.title.split(":")[0]} 
